...
export function MetricChart({ points, step, unavailability, loading = false }: MetricChartProps) {
  if (unavailability) return <UnavailabilityBlock code={unavailability} />
  return <ChartCanvas points={chartData(points)} step={step} loading={loading} />
}

function ChartCanvas({ points, step, loading }: { points: [number, number | null][]; step: string; loading: boolean }) {
  ...
  useEffect(() => {
    ...
    chart.setOption({
      ...
      yAxis: { type: 'value', name: '连接数', min: 0, splitLine: { lineStyle: { color: '#e1e0d9', width: 1 } } },
      series: [{ name: '总连接数', type: 'line', data: points, connectNulls: false, showSymbol: false, lineStyle: { color: '#2a78d6', width: 2 } }],
    })
  }, [points])

  return (
    <figure style={{ margin: 0, opacity: loading ? 0.55 : 1 }} aria-label="PostgreSQL 总连接数趋势">
      <div ref={ref} style={{ height: 340, width: '100%' }} />
      <figcaption>实际粒度：{step}</figcaption>
      <details>
        <summary>查看数据表</summary>
        <table><thead><tr><th>时间</th><th>连接数</th></tr></thead><tbody>{points.map(([timestamp, value]) => <tr key={timestamp}><td>{new Date(timestamp).toISOString()}</td><td>{value == null ? '缺数' : value}</td></tr>)}</tbody></table>
      </details>
    </figure>
  )
}
