...
type QueryStatisticsView =
  ...
  | { kind: 'unavailable'; code: components['schemas']['Unavailability']; title: string; description: string }
...
function unavailableView(code: components['schemas']['Unavailability']): Extract<QueryStatisticsView, { kind: 'unavailable' }> {
  switch (code) {
    ...
    case 'FEATURE_DISABLED':
      return { code, title: '未启用', description: '目标数据库未提供可用的 pg_stat_statements 能力，请检查扩展与预加载配置。', kind: 'unavailable' }
    case 'PERMISSION_DENIED':
      return { code, title: '权限不足', description: '监控账号无法读取查询统计，请核对监控角色权限。', kind: 'unavailable' }
    case 'COUNTER_RESET':
      return { code, title: '统计已重置', description: '统计计数器已重置，等待下一次完整采集后再查看排行。', kind: 'unavailable' }
    case 'NO_DATA_IN_RANGE':
      return { code, title: '区间内无记录', description: '最近一次查询统计快照没有可排行的记录。', kind: 'unavailable' }
    case 'NO_SAMPLES_YET':
    case 'STALE':
