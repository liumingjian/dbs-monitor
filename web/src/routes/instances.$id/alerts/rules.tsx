...
function AlertRulesPage() {
  ...
  return <Space direction="vertical" size="large" style={{ width: '100%' }}>
    ...
    <Space className="workbench-heading" wrap>
      <div>
        <Typography.Title level={2} style={{ margin: 0 }}>{instanceQuery.data?.name ?? '实例工作台'}</Typography.Title>
    ...
    <Tabs activeKey="alerts" items={[
      ...
      { key: 'alerts', label: <><BellOutlined /> 告警</> },
      { key: 'collection', label: '采集管理', disabled: true },
    ]} />
    <Tabs activeKey="rules" items={[
      { key: 'rules', label: '告警规则' },
      { key: 'current', label: '当前告警', disabled: true },
      { key: 'history', label: '告警历史', disabled: true },
    ]} />
