...
import { Alert, Button, DatePicker, Form, Input, Modal, Popconfirm, Select, Space, Table, Tabs, Tag, Tooltip, Typography } from 'antd'
...
export function parseMaintenanceSearch(search: Record<string, unknown>): MaintenanceSearch {
  return typeof search.instance_id === 'string' && search.instance_id !== ''
...
export function groupMaintenanceWindows(windows: MaintenanceWindow[]): Record<MaintenanceStatus, MaintenanceWindow[]> {
  ...
  for (const window of windows) {
    switch (window.status) {
      ...
      case 'SCHEDULED':
      case 'ENDED':
        grouped[window.status].push(window)
        break
      default:
        assertNever(window.status)
    }
  }
  return grouped
}

function MaintenanceSettingsPage({ initialInstanceId, openInitially = false }: { initialInstanceId?: string; openInitially?: boolean }) {
  ...
  function openEditor(window?: MaintenanceWindow) {
    setEditing(window ?? null)
    form.resetFields()
    form.setFieldsValue(window ? {
      instance_ids: window.instance_ids,
      time_range: [dayjs(window.starts_at), dayjs(window.ends_at)],
  ...
  function save(values: MaintenanceForm) {
    ...
    const options = {
      onSuccess: () => {
        setOpen(false)
        setFeedback({ type: 'success' as const, text: editing ? '维护窗口已更新' : '维护窗口已创建' })
        void windowsQuery.refetch()
      },
      onError: (error: unknown) => setFeedback({ type: 'error' as const, text: apiErrorMessage(error, '保存维护窗口失败') }),
    }
    if (editing) updateMutation.mutate({ params: { path: { id: editing.id } }, body }, options)
    else createMutation.mutate({ body }, options)
  }

  function endWindow(window: MaintenanceWindow) {
    endMutation.mutate({ params: { path: { id: window.id } } }, {
      onSuccess: () => void windowsQuery.refetch(),
      onError: (error) => setFeedback({ type: 'error', text: apiErrorMessage(error, '提前结束维护窗口失败') }),
    })
  }

  function removeWindow(window: MaintenanceWindow) {
    deleteMutation.mutate({ params: { path: { id: window.id } } }, {
      onSuccess: () => void windowsQuery.refetch(),
  ...
  return (
    <Space orientation="vertical" size="large" style={{ width: '100%' }}>
      ...
      <Tabs items={[
        { key: 'ACTIVE', label: `生效中 ${grouped.ACTIVE.length}`, children: <MaintenanceTable windows={grouped.ACTIVE} instances={instances} canManage={canManage} onEdit={openEditor} onEnd={endWindow} onDelete={removeWindow} /> },
        { key: 'SCHEDULED', label: `未开始 ${grouped.SCHEDULED.length}`, children: <MaintenanceTable windows={grouped.SCHEDULED} instances={instances} canManage={canManage} onEdit={openEditor} onEnd={endWindow} onDelete={removeWindow} /> },
        { key: 'ENDED', label: `已结束 ${grouped.ENDED.length}`, children: <MaintenanceTable windows={grouped.ENDED} instances={instances} canManage={canManage} onEdit={openEditor} onEnd={endWindow} onDelete={removeWindow} /> },
      ]} />
      <Modal title={editing ? '编辑维护窗口' : '新建维护窗口'} open={open} footer={null} destroyOnHidden onCancel={() => setOpen(false)}>
        <Form<MaintenanceForm> form={form} layout="vertical" onFinish={save}>
          ...
          <Form.Item name="reason" label="原因" rules={[{ required: true, whitespace: true }]}><Input.TextArea rows={3} /></Form.Item>
...
function MaintenanceTable({ windows, instances, canManage, onEdit, onEnd, onDelete }: {
  ...
  return <Table<MaintenanceWindow>
    ...
    columns={[
      ...
      { title: '实例', render: (_, window) => window.instance_ids.map((id) => instanceNames.get(id) ?? id).join('、') },
      { title: '状态', width: 90, render: (_, window) => <Tag color={statusColor(window.status)}>{statusLabel(window.status)}</Tag> },
      { title: '开始时间', width: 180, render: (_, window) => formatTime(window.starts_at) },
      { title: '结束时间', width: 180, render: (_, window) => formatTime(window.ends_at) },
      { title: '创建人', width: 130, render: (_, window) => window.created_by.slice(0, 8) },
      {
        title: '操作', width: 150, fixed: 'right', render: (_, window) => (
          <Space>
            <Tooltip title={window.status === 'ENDED' ? '已结束窗口不可编辑' : '编辑维护窗口'}>
              <span><Button aria-label={`编辑 ${window.reason}`} icon={<EditOutlined />} disabled={!canManage || window.status === 'ENDED'} onClick={() => onEdit(window)} /></span>
            </Tooltip>
            <Popconfirm title="提前结束此维护窗口？" disabled={!canManage || window.status !== 'ACTIVE'} onConfirm={() => onEnd(window)}>
              <Tooltip title="提前结束"><span><Button aria-label={`提前结束 ${window.reason}`} icon={<StopOutlined />} disabled={!canManage || window.status !== 'ACTIVE'} /></span></Tooltip>
            </Popconfirm>
            <Popconfirm title="删除此维护窗口？" disabled={!canManage} onConfirm={() => onDelete(window)}>
              <Tooltip title="删除维护窗口"><Button aria-label={`删除 ${window.reason}`} danger icon={<DeleteOutlined />} disabled={!canManage} /></Tooltip>
            </Popconfirm>
          </Space>
        ),
      },
