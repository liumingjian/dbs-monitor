...
func TestStatDatabaseRateState(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			observations: []statDatabaseSnapshot{
				{observedAt: base, counters: [statDatabaseCounterCount]float64{100, 20, 1_000, 500, 10, 1_000}},
				{observedAt: base.Add(10 * time.Second), counters: [statDatabaseCounterCount]float64{130, 30, 1_200, 550, 12, 1_500}},
			},
			want: []statDatabaseExpectation{
				{},
		...
		},
		{
			...
			observations: []statDatabaseSnapshot{
				{observedAt: base, counters: [statDatabaseCounterCount]float64{100, 20, 1_000, 500, 10, 1_000}},
				{observedAt: base.Add(10 * time.Second), counters: [statDatabaseCounterCount]float64{5, 2, 100, 50, 1, 100}},
				{observedAt: base.Add(20 * time.Second), counters: [statDatabaseCounterCount]float64{15, 4, 120, 60, 2, 200}},
			},
			want: []statDatabaseExpectation{
				{},
				{counterReset: true},
		...
		},
		{
			...
			observations: []statDatabaseSnapshot{
				{observedAt: base, counters: [statDatabaseCounterCount]float64{100, 20, 1_000, 500, 10, 1_000}},
				{observedAt: base.Add(5 * time.Minute), counters: [statDatabaseCounterCount]float64{400, 80, 4_000, 2_000, 40, 4_000}},
				{observedAt: base.Add(time.Minute), counters: [statDatabaseCounterCount]float64{150, 30, 1_500, 750, 15, 1_500}},
				{observedAt: base.Add(5*time.Minute + 10*time.Second), counters: [statDatabaseCounterCount]float64{410, 82, 4_100, 2_050, 41, 4_100}},
			},
			want: []statDatabaseExpectation{
				{},
				{sampleCount: 7, values: map[metric.MetricID]float64{
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			for index, observation := range tt.observations {
				...
				for metricID, wantValue := range want.values {
					if value, exists := got[metricID]; !exists || value != wantValue {
						t.Errorf("observation %d metric %s = %v, %t; want %v", index, metricID, value, exists, wantValue)
					}
				}
				if len(got) != want.sampleCount {
					t.Errorf("observation %d samples = %v, want %v", index, got, want)
				}
			}
		})
	}
