	...
	for _, target := range targets {
		...
		for _, task := range tasks {
			...
			if !exists || entry.template.interval != template.interval {
				entry.nextDue = initialDue(now, template)
			}
...
func (scheduler *centralScheduler) refreshDiskHealth(now time.Time) {
	if scheduler.service.health == nil || scheduler.service.diskPath == "" {
		return
	}
	usage, err := disk.Usage(scheduler.service.diskPath)
	if err != nil {
		scheduler.service.health.Update(now, platformhealth.DiskUnavailableSource(scheduler.service.health.DiskLevel()))
		return
	}
...
func initialDue(now time.Time, run scheduledRun) time.Time {
	if isCapabilitySnapshotTask(run.task) {
		return now
	}
	return nextDueAfter(now, initialPhase(run.key.instanceID, run.key.taskID, run.interval), run.interval)
}
