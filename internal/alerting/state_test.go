...
func TestStepStateTransitions(t *testing.T) {
	tests := []struct {
		...
		{name: "ok breach fires immediately", current: Snapshot{State: OK}, input: Breaching, want: Snapshot{State: FIRING, BreachCount: 1}},
		...
		{name: "recovered breach fires immediate lifecycle", current: Snapshot{State: RECOVERED}, input: Breaching, want: Snapshot{State: FIRING, BreachCount: 1}},
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			triggerCount := 2
			if test.name == "ok breach fires immediately" || test.name == "recovered breach fires immediate lifecycle" {
				triggerCount = 1
			}
