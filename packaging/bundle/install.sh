...
case "$install_root/etc" in
  "$data_dir"|"$data_prefix/"*)
    echo "data directory must not contain $install_root/etc; database backups and the credential keyring must remain separate" >&2
    exit 1
    ;;
esac
case "$data_dir" in
  "$install_root/etc"|"$install_root/etc/"*)
    echo "data directory must not be inside $install_root/etc; database backups and the credential keyring must remain separate" >&2
    exit 1
    ;;
esac
