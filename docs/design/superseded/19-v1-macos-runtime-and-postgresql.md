...
发布资产的外层格式、签名、公证、下载渠道和构建验证由 [macOS 构建、验证与发布路线 #101](https://github.com/liumingjian/dbs-monitor/issues/101) 决定；本文只规定无论采用何种外层资产都必须满足的运行契约。
...
| 下游 | 必须承接的内容 |
|---|---|
| [#101 macOS 构建、验证与发布路线](https://github.com/liumingjian/dbs-monitor/issues/101) | 决定外层资产、签名/公证和 CI；构建并审计原生 PG 17 与非系统动态库，保存上述干净机证据 |
