...
**开发者面对的命令仍然只有两条（`make check` / `make check-full`），`make acceptance` 与 `release-gate` 定性为非开发者入口，[10](10-ai-guardrails-and-verification.md) D1 不需要 supersede；104 条验收矩阵每次 push `main` 全量跑、红即阻断，但在 CI 里是独立 job；被监控 PG13–17 矩阵只跑采集与能力探测子集、跑在手工发起的发布门；平台库必须是 17 落成结构守卫 B14（既查连上的库也扫 compose 与 workflow 的 image tag）；供应链扫描的归属让给先落地的 [29](29-production-security-boundary.md) D7、本票只定它不进 `evidence` 也不参与 `verdict`；RT-C 是记录门不是阈值门，缺数据即 NO-GO；三份证据合成唯一 `verdict`，判定规则仍写死在生成器里，缺失与 SHA 不一致是两种不同的失败；GitHub runner 上跑绿的验收一律标 `provisional` 且永不出 `GO`——能进 `verdict` 的证据只能来自真机 Linux。**
...
| `release-gate` | CI，tag push 触发（[27](27-v1-deliverables-and-candidate-provenance.md) D6） | 否 |

1. **`make check-full` 作为命令仍然包含 `make acceptance`**——交付团队与真机验收要的是「一条命令复跑全部」，这是 [15](15-ci-and-release-pipeline.md) D1「本地与 CI 复用同一组 `make` 命令」的直接要求。
2. **在 CI 里 `acceptance` 拆成独立 job**，与 `check-full` 并行（见 D2）。
3. [10](10-ai-guardrails-and-verification.md) D1「不许加第三层」**不 supersede，继续成立**：它的论证目标是「不让执行者面对判断题」，而判断题的数量取决于**开发者要记住几条命令**，不取决于 CI 有几个 job。`make acceptance` 与 `release-gate` 开发者从不手敲。
...
3. 「CI 完全不跑、只在真机人工跑」会让百余条退化成人工检查表，[10](10-ai-guardrails-and-verification.md)「把规范做成结构，而不是做成约定」当场失守。
...
## 5. D5 · 供应链扫描：归属让给 [29](29-production-security-boundary.md) D7，本票只定它怎么进报告

**结论**

1. **扫描的归属与阻断语义以 [29](29-production-security-boundary.md) D7 为准，本票不推翻**：`govulncheck` 进 `check-full` 且阻断；`npm audit` 进 `check-full` 只报告不阻断；两者都不进矩阵；**`release-gate` 不重跑扫描、只读报告**。
2. 本票只定 [29](29-production-security-boundary.md) 明确移交过来的那一件事——**结果怎么进 Go/No-Go 报告**：
   - `acceptance-report.json` 的 `evidence` 块**不设 vuln 子块**，扫描结果**不参与 `verdict`**。
   - `govulncheck` 的阻断作用**已经完整体现在「该 SHA 有成功的 `check-full`」这一条里**（[27](27-v1-deliverables-and-candidate-provenance.md) D6），报告不必重复记账。
   - `npm audit` 的输出进薄壳 Markdown 结论页的「遗留风险」段（[27](27-v1-deliverables-and-candidate-provenance.md) D5.2），**不得影响 `verdict`**——它本来就是报告不是门。
3. **不产 SBOM**：[27](27-v1-deliverables-and-candidate-provenance.md) D2 已定 v1 不发布任何二进制、交付团队自建，SBOM 在这个形态下没有消费者。
4. Dependabot 开启（PR 形式的升级建议不是门）。

**记账：本票原本推荐把扫描拆成独立 `vuln-scan` workflow 并明确不计入 `release-gate`**，理由是「新披露一个 CVE，昨天还绿的候选今天重跑必红，tag 就此打不出来」。[29](29-production-security-boundary.md) D7 用另一条路径解掉了同一个问题——**`release-gate` 只读报告、不重跑扫描**，那么已记录的成功 `check-full` 不会被后来的 CVE 追溯作废。两条路径解的是同一个失效面，[29](29-production-security-boundary.md) 先落地且顺带解决了离线构建机上「跳过即绿」这一条本票没看到的退化，**故本票让步，不新开第二套安排**。`npm audit` 只报告的理由（传递依赖与 devDependency 噪音大，天天红的门等于没有门）也强于本票原推荐的 `--audit-level=high` 阻断。
...
- **不设自动通过阈值**。数字进报告，Go/No-Go 由人在薄壳 Markdown（[27](27-v1-deliverables-and-candidate-provenance.md) D5.2）里判。
...
**结论**：扩 `acceptance-report.json` 的 schema，顶层新增 `evidence` 块，**判定规则仍然写死在生成器里**（[27](27-v1-deliverables-and-candidate-provenance.md) D5 不变）。
...
1. **101 条硬底**任一非 `pass` → `NO-GO`（[29](29-production-security-boundary.md) `SEC-1..10` 落地后的终态：条目 104 / 硬底 101，本票不动）。
...
1. [27](27-v1-deliverables-and-candidate-provenance.md) D5 的核心论证是「**规则若跟着报告走，改报告就能改结论**」。把「三份都要在」这条规则挪进 `release-gate`，等于让规则分散在两个执行者里，同一个论证同样适用。
...
1. **不由 tag 触发**：`release-gate` 校验的前提是报告**已经存在**。tag 触发会造成「打 tag → 跑证据 → 补报告 → 重跑 gate」的环形依赖；而 rc tag 的语义 [27](27-v1-deliverables-and-candidate-provenance.md) D6 已经定死为「**这是一个被正式评判过的候选**」，不能兼作「请开始评判」。
2. **不定时跑**：每晚跑 `main` 打的是移动靶，产出的证据不绑定任何候选——正好是 D7 规则 3 要防的那件事。
3. 一般化为一条纪律：**一切证据类 workflow 一律以 SHA 为入参手工发起**，与「候选身份 = SHA」（[27](27-v1-deliverables-and-candidate-provenance.md) D1）同构。
...
**理由**：地图 Notes 第 4 条定死「v1 的 Go/No-Go 证据必须在真实 Linux 上产出」，而 GitHub runner 字面上也是 Linux——这两条不冲突但显然不是一个意思，必须在本票写死判据，否则日后一定有人拿 CI 的绿当验收通过。具体差异是可点名的：runner 是容器化、**无 systemd**、cgroup 分母被限、磁盘与时钟都不是客户形态。于是 [26](26-data-and-recovery-gate.md) 的重启恢复、[25](25-master-key-provenance-and-startup-failure.md) 的 `0600` 权限与 root 预建目录、[23](23-v1-acceptance-entries-c.md) 的 `AC-05-S4` 观感门，在那上面跑绿说明不了任何事。
...
1. 维持 [27](27-v1-deliverables-and-candidate-provenance.md) D3：`main` 的 required status check **只有 `check`**。`check-full`（含漏洞扫描）与 `acceptance` 全部是事后门，**`main` 可以红**。
2. 配套纪律：**`main` 红时，除修复外不合入任何 PR、不打任何 tag。**
3. 机器兜底：`release-gate` 拿不到该 SHA 成功的 `check-full` 就发不出 tag（[27](27-v1-deliverables-and-candidate-provenance.md) D6），这已经是结构性的。

**理由**：把 `check-full` 或 `acceptance` 提为 required check，会让每个 PR 等 40–60 分钟。单人仓库下的真实结局是**本地跑一遍就直接 push 到 `main` 绕开 PR**——与 [27](27-v1-deliverables-and-candidate-provenance.md) 拒绝 required approval 是同一个论证：**把会被绕过的规则伪装成生效的规则，比没有规则更坏**。
...
1. [10](10-ai-guardrails-and-verification.md) D1 的 **≤120 秒预算与其重新触发条件一字不改**。
2. 新增的 B11 / B12 / B14 三条守卫全部进 `make check`：两条静态扫描 + 一次库查询 + 一次 tag 文本扫描，量级在秒。
3. **`make acceptance` 绝不出现在 `make check` 里。**
4. 若 B 栏新增把快层推过 120 秒，仍走 [10](10-ai-guardrails-and-verification.md) §15 既定处置——**先产出分包耗时数据、定位大户再挪，禁止为过预算整体搬迁**。
...
1. [10](10-ai-guardrails-and-verification.md) §3.3 的 B 栏**新增一列「跑在哪层」**。
2. B 栏新增四条：**B11**（禁止对业务表直插，[20](20-v1-acceptance-matrix.md) D4）、**B12**（`test_ref` 覆盖漂移门，[20](20-v1-acceptance-matrix.md) D6.6）、**B13**（安全头 golden，[29](29-production-security-boundary.md) D9）、**B14**（本票 D4）。**本票的平台库守卫编号为 B14 而非 B13**——`B13` 已被先落地的 [29](29-production-security-boundary.md) 占用。
3. A 栏新增三条：**A10**（三档角色可见性 / 写能力判定表）、**A11**（三条内置规则受限语义判定表）——兑现 [24](24-v1-acceptance-entries-d.md) D10；**A12**（必须可归因的写操作登记表）——兑现 [29](29-production-security-boundary.md) D5.2。
4. **不开 C 栏。** `acceptance`、五版本矩阵、RT-C、漏洞扫描、`release-gate` 只登记在本文 §14 的门总表里。

**理由**：A/B 两栏的准入判据（[10](10-ai-guardrails-and-verification.md) §3.1 三问）要求「唯一实现点、可表驱动或可结构化扫描」，`acceptance` 与 RT-C 都不满足；硬塞会稀释 [10](10-ai-guardrails-and-verification.md) 自己那句「**清单的价值来自它短**」。而新开一栏等于在 `10` 号里再造一份门总表，与本文构成第二份真相。加一列成本最低且诚实——B 栏本来就已经不全在一层（B11 的射程覆盖 `test/` 下只在 `check-full` 里执行的东西）。
...
| G6 | `govulncheck`（阻断）+ `npm audit`（只报告） | 含在 `check-full` 内（[29](29-production-security-boundary.md) D7） | push `main` / 手动 | `govulncheck` 是 | 同 G2；`release-gate` **不重跑扫描、只读报告** | 否 |
...
**理由**：[27](27-v1-deliverables-and-candidate-provenance.md) 已定「不建交付前检查表（第二份真相），勾选表归 #114 且应由报告承载」——本票兑现的方式就是**不建**。把门总表放进 `matrix.yaml` 同样不行：[20](20-v1-acceptance-matrix.md) 定的矩阵射程是**产品语义**，[27](27-v1-deliverables-and-candidate-provenance.md) 已用同一条理由把 `release-gate` 挡在矩阵外。**门总表是决策文档里的表，判定是生成器里的代码，执行结果是 JSON——三者各司其职、无一处可勾选。**
...
8. 在 [10](10-ai-guardrails-and-verification.md) §3.2/§3.3 登记 A10 / A11 / A12 / B11 / B12 / B13 / B14，并给 B 栏加「跑在哪层」一列。**本票随文一并落地**，一并结清 [24](24-v1-acceptance-entries-d.md) D10 与 [29](29-production-security-boundary.md) §11 原本挂在片⑧实现票上的登记项（登记的约束力由「兑现时机」列承载，提前登记不改变兑现时点）。
9. `main` 分支保护按 [27](27-v1-deliverables-and-candidate-provenance.md) D3 配置；required check 保持只有 `check`，**不因本票新增 job 而扩充**。
...
| 文档 | 处置 |
|---|---|
| [10](10-ai-guardrails-and-verification.md) D1 | **不 supersede**。加适用面澄清（D1）：「两层」约束的是开发者面对的命令数，不是 CI job 数。预算与重新触发条件一字不改（D12） |
| [10](10-ai-guardrails-and-verification.md) §2.1 `datlocprovider` | 由 B14 落地（D4）。规定本身不改 |
| [10](10-ai-guardrails-and-verification.md) §3.2 / §3.3 | **只增不改**：加两条 A 栏、三条 B 栏、一列「跑在哪层」 |
| [15](15-ci-and-release-pipeline.md) D1 | 保留，是本票地基 |
| [15](15-ci-and-release-pipeline.md) D2 触发表 | **不 supersede，只增不改**。现有四行没有一行是错的，全部仍成立；新增两行（`acceptance` 独立 job、`release-evidence` 手工触发）**写在本文 §14 而不是回填 `15` 号** |
| [15](15-ci-and-release-pipeline.md) D3.1 / D3.2 | 仍成立，但**执行者已由人变机器**（[27](27-v1-deliverables-and-candidate-provenance.md) D6 的 `release-gate`）。记账，不改写 |
| [15](15-ci-and-release-pipeline.md) D3.3 / D4 / D5 | 早已由 [18](18-v1-delivery-boundary-bs-binary.md) 作废，本票不复活 |
| [15](15-ci-and-release-pipeline.md) D3.4 | 保留：最小权限、不用个人长期 token |
...
**另记一笔**：`docs/design/` 中存在两份 27 号（`27-v1-deliverables-and-candidate-provenance.md` 与 `27-external-postgres-prerequisites.md`），[#110](https://github.com/liumingjian/dbs-monitor/issues/110) 与 [#116](https://github.com/liumingjian/dbs-monitor/issues/116) 撞号。[29](29-production-security-boundary.md) 为避开撞号跳过了 `28-`，本文补上该空缺。撞号本身不在本票射程内，**不原地改名**（改名会打断所有既有互链），留待专门处置。
...
| 被否决的方案 | 理由 |
|---|---|
| 正式 supersede [10](10-ai-guardrails-and-verification.md) D1 改写成三层 | 「两层」约束的是开发者的判断题数量，CI job 数与之无关。改写会让后来的会话以为可以随便加层 |
...
| 漏洞扫描进 PR 门 | 「什么都没改却红了」是教会所有人绕过门的最快机制（[29](29-production-security-boundary.md) D7 同侧，未进 PR 门） |
| 把漏洞扫描拆成独立 `vuln-scan` workflow 并排除出 `release-gate` | **本票原推荐，已撤回**：[29](29-production-security-boundary.md) D7 先落地，且「`release-gate` 只读报告不重跑」已解掉同一个失效面，另建 workflow 只会造出第二套安排 |
...
| 在 [10](10-ai-guardrails-and-verification.md) 新开 C 栏「发布线门禁」 | 在 `10` 号里再造一份门总表，与本文构成第二份真相 |
| 另出 `docs/deploy/go-no-go-checklist.md` 勾选表 | 正是 [27](27-v1-deliverables-and-candidate-provenance.md) 点名拒绝的第二份真相 |
| 门总表进 `matrix.yaml` | 违反矩阵射程 = 产品语义，同 [27](27-v1-deliverables-and-candidate-provenance.md) 挡 `release-gate` 的理由 |
| supersede [15](15-ci-and-release-pipeline.md) D2 整条并重写触发表 | 现有四行没有一行是错的；supersede 没错的条款只会让后来者去比对两份表找差异 |
