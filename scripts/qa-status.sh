...
node -e '
const fs = require("fs")
const instances = JSON.parse(fs.readFileSync(process.argv[1], "utf8")).items
...
id=$(node -e 'process.stdout.write(JSON.parse(require("fs").readFileSync(process.argv[1],"utf8")).items[0].id)' "$state/instances.json")
